...
GENTOO_RELEASES_URL=https://mirror.init7.net/gentoo/releases/

GENTOO_ARCH=amd64
GENTOO_VARIANT=amd64

TARGET_DISK=$(lsblk -N -n -o path)
...
# Creating partitions.
echo sfdisk $TARGET_DISK -a
echo type=linux | sfdisk $TARGET_DISK -a
...
# STAGE3_PATH_URL=https://gentoo.osuosl.org/releases/amd64/autobuilds/current-stage3-amd64-llvm-systemd/latest-stage3-amd64-llvm-systemd.txt
STAGE3_PATH_URL=https://mirror.init7.net/gentoo/releases/amd64/autobuilds/current-stage3-amd64-llvm-systemd/latest-stage3-amd64-llvm-systemd.txt
STAGE3_PATH=$(curl -s $STAGE3_PATH_URL | grep stage | cut -d" " -f1)
STAGE3_URL=$GENTOO_RELEASES_URL/$GENTOO_ARCH/autobuilds/current-stage3-amd64-llvm-systemd/$STAGE3_PATH
...
# USE
echo 'USE="systemd lvm networkmanager dbus bluetooth -bindist"' >> /etc/portage/make.conf
...
# echo 'dev-lang/python -bluetooth sqlite' >> /etc/portage/package.use/python
...
# todo root_keydev command not working
echo -e GRUB_CMDLINE_LINUX=\"dolvm crypt_root=UUID=$(blkid -s UUID -t TYPE=crypto_LUKS -o value) root=/dev/mapper/vg-1-root root_keydev=$(blkid -s UUID -L SYSTEM) root_key=keyfile key_timeout=4 $grubCMDLine\" >> /etc/default/grub
...
emerge =app-admin/ansible-5.10.0
emerge dev-vcs/git app-admin/sudo
